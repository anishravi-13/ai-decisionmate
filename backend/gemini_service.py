...
{{
  ...
  "products": [
    {{
      "id": "prod_1",
      "name": "Exact Recommended Item / Course / Model",
      "price": "Price in INR (e.g. ₹48,999 (est.))",
      "rating": 4.7,
      "specs": {{"type": "Primary Selection", "feature": "Key highlight"}},
      "match_pct": 98.0,
      "why_match": "Direct primary recommendation aligning with your exact requirements.",
      "trade_offs": ["Key consideration before purchase"]
    }},
    {{
      "id": "prod_2",
      "name": "Top Related Alternative or Essential Accessory",
      "price": "Price in INR",
      "rating": 4.5,
      "specs": {{"type": "Alternative Choice", "feature": "Key highlight"}},
      "match_pct": 88.0,
      "why_match": "High-value direct alternative in the same category.",
      "trade_offs": ["Alternative compromise"]
    }}
  ],
  "follow_up_questions": [
...
"""
    ...
    result = _build_decision_result(data, query, "general")
    if not result.products:
        try:
            from decision_engine import _generate_product_cards, extract_budget
            b_val = extract_budget(query) or 0.0
            cards = _generate_product_cards(result.category, {"budget": b_val}, b_val, 1, recommendation=result.recommendation)
            if cards:
...
"""
    ...
    # Attach demo catalog products if not already generated by AI
    if not result.products:
        try:
            ...
            cards = _generate_product_cards(category, answers, budget_val, quantity_val, recommendation=result.recommendation)
            if cards:
                result.products = cards
        except Exception:
            pass
    ...
    for p in data.get("products", []):
        try:
            products.append(ProductCard(
                id=str(p.get("id", f"gem_prod_{len(products)+1}")),
                name=str(p.get("name", "Recommended Option")),
                price=p.get("price"),
                rating=float(p.get("rating", 4.7)),
                specs=dict(p.get("specs", {})),
                match_pct=float(p.get("match_pct", 96.0)),
                why_match=str(p.get("why_match", "Directly matches your primary recommendation and stated criteria.")),
                trade_offs=list(p.get("trade_offs", [])),
                demo_data=True,
            ))
        except Exception:
            pass
